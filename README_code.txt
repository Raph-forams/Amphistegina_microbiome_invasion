================================================================================
README: MICROBIOME ANALYSIS CODE
================================================================================
...
This repository contains the complete analysis workflow for characterizing 
microbiome composition of Amphistegina lobifera across an invasion gradient 
from the Red Sea to the Mediterranean Sea. The analysis includes:
...
The code is designed to be fully reproducible and generates all figures and 
statistical tables presented in the manuscript.
...
project_root/
├── code/
│   └── analysis_script.R          # This analysis script
├── data/
...
Edit line 23 in the script to match your local directory:
  setwd("/path/to/your/project_root/code")

Or comment out this line and set the working directory manually in RStudio:
...
Option B - Run interactively (recommended for first time):
  Open script in RStudio and run section by section using Ctrl+Enter
...
- Calculate saturation metrics:
  * Rarefaction curve slopes
  * Richness gain in final 20% of sequencing
- Generate Figure S5 (saturation assessment)

SECTION 6: COMMUNITY COMPOSITION ANALYSIS (FIGURE 3)
- Alpha diversity (Inverse Simpson index)
  * Kruskal-Wallis tests (site, depth, substrate)
  * Pairwise Wilcoxon comparisons with FDR correction
  * Export Table S1 (alpha diversity tests)
  * Export Table S2 (pairwise comparisons)
- Beta diversity (Bray-Curtis dissimilarity)
  * NMDS ordination (k=3 for robustness)
  * PERMANOVA (full factorial: Site × Depth × Substrate)
  * Pairwise site comparisons
  * Betadispersion tests
  * Export Table S3 (PERMANOVA results)
  * Export Table S4 (pairwise PERMANOVA)
  * Export Table S5 (betadispersion tests)
- Community correlation (Mantel test)
  * Export Table S6 (Mantel test results)
- Generate Figure 3 panels (A-D for diatoms and prokaryotes)

SECTION 7: TAXONOMIC COMPOSITION ANALYSIS (FIGURES 2 & 4)
- Overall community composition (Figure 2)
  * Phylum/Division level comparisons
  * Sample type differentiation
- Diatom symbiont analysis (Figure 4)
  * Genus-level composition
  * ASV-level diversity within genera
  * Site-specific patterns
  * Spectral color palette for visualization

SECTION 8: FUNCTIONAL PROFILING ANALYSIS (FIGURE 5)
- Load FAPROTAX output
- Assignment rate analysis
  * Generate Figure S6 (assignment rates by sample type)
  * Export Table S8 (assignment statistics)
- NMDS ordination of functional profiles
  * Generate Figure 5A (all sample types)
  * Generate Figure 5B (foraminifera only)
- PERMANOVA and ANOSIM tests
  * Export Table S9 (PERMANOVA results)
  * Export Table S10 (ANOSIM and betadispersion)
- Functional enrichment analysis
  * Overall enrichment (Foraminifera vs Environment)
  * Generate Figure 5C (volcano plot)
  * Export Table S11 (overall enrichment)
- Site-specific enrichment
  * Generate Figure S7 (individual site volcano plots)
  * Export Table S12 (consistency across sites)
  * Export Table S13 (site-specific details)
...
- Sequential removal order: Rare → Taxonomy → Contaminants
...
- Enrichment threshold: >2× fold-change
- Depletion threshold: <0.5× fold-change
...
Technical replicates (PCR replicates from same DNA extraction) are merged by 
summing reads. This approach:
- Increases sequencing depth per sample
- Reduces technical variance
- Preserves biological signal
- Validated by low Bray-Curtis dissimilarity between replicates
...
SOLUTION: Verify working directory is set correctly and data/ folder exists 
          in parent directory

ISSUE: "Package not found"
SOLUTION: Install missing packages: install.packages("package_name")

ISSUE: "Memory allocation error"
SOLUTION: Close other programs, increase R memory limit:
          memory.limit(size=16000)  # Windows only

ISSUE: "Duplicate rownames in phyloseq"
SOLUTION: This is handled automatically by using distinct() before creating 
          phyloseq objects

ISSUE: Figures look different from manuscript
SOLUTION: Ensure you're using the same data version and R version ≥4.3.3

ISSUE: Very slow execution
SOLUTION: Rarefaction and distance calculations are computationally intensive.
          Consider running on a machine with more cores or reduce dataset size 
          for testing.
...
Data repository: [ZENODO DOI]

For questions about the code, contact:
Raphaël Morard (rmorard@marum.de)

================================================================================
LICENSE
================================================================================

[LICENSE INFORMATION - e.g., MIT, GPL-3, CC-BY-4.0]

This code is provided "as is" without warranty of any kind. Users are free to 
modify and adapt the code for their own research, provided appropriate citation 
is given.
...
- Initial release
- Complete workflow from data import to figure generation
- All supplementary tables exported
- Validated against manuscript figures

Future updates will be tracked at: [GITHUB URL]

================================================================================
ACKNOWLEDGMENTS
================================================================================

We thank the developers and maintainers of all R packages used in this analysis, 
particularly:
