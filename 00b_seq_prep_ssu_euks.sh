...
WDIR="/storage/hdd1/chh/Debora_amplicons/Single_euks"
cd $WDIR
...
for library in $(sed '1d' Eukaryotes_single_forams_library_info.txt | cut -f6 | sort | uniq)
do
  sed '1d' Eukaryotes_single_forams_library_info.txt | awk -v FS="\t" -v OFS="\t" -v library=${library} '$6 == library' > "Library_"${library}"_index_primer.txt"
done
...
do
  ...
  do
    ...
    cutadapt -j 2 -O ${OLP} --no-indels -e 0 -g ${FWD} -G ${REV} --discard-untrimmed -o Demux/${SID}"_fr_R1.fastq.gz" -p Demux/${SID}"_fr_R2.fastq.gz" Libraries/"Library"${LIB}"_S"${LIB}"_L001_R1_001.fastq.gz" Libraries/"Library"${LIB}"_S"${LIB}"_L001_R2_001.fastq.gz" > Logfiles/${SID}".demux_fr.log" 2>&1
    
    cutadapt -j 2 -O ${OLP} --no-indels -e 0 -g ${REV} -G ${FWD} --discard-untrimmed -o Demux/${SID}"_rf_R1.fastq.gz" -p Demux/${SID}"_rf_R2.fastq.gz" Libraries/"Library"${LIB}"_S"${LIB}"_L001_R1_001.fastq.gz" Libraries/"Library"${LIB}"_S"${LIB}"_L001_R2_001.fastq.gz" > Logfiles/${SID}".demux_rf.log" 2>&1 
  done < ${i}
done
